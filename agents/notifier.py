...
class BotState:
    ...
    trade_executor_callback: Callable = None
...
class ApexNotifier:
    ...
    async def handle_trade_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        _, action, trade_id = query.data.split("_", 2)
        ...
        trade_data = BotState.pending_trades.pop(trade_id)
    ...
    async def _safe_execute(self, trade_data):
        try:
            ...
            if asyncio.iscoroutinefunction(BotState.trade_executor_callback):
                await BotState.trade_executor_callback(trade_data)
            else:
                # Run sync in thread pool to not block loop
                loop = asyncio.get_event_loop()
                await loop.run_in_executor(None, lambda: BotState.trade_executor_callback(trade_data))
